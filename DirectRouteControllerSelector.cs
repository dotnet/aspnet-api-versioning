    ...
    sealed class DirectRouteControllerSelector : ControllerSelector
    {
        ...
        static HttpControllerDescriptor ResolveController(CandidateAction[] directRouteCandidates, ApiVersion requestedVersion)
        {
            ...
            var matches = directRouteCandidates
                .Where(c => c.ActionDescriptor.GetApiVersions().Contains(requestedVersion));

            matches = matches.Union(directRouteCandidates
                    .Where(cd => cd.ActionDescriptor.ControllerDescriptor.GetDeclaredApiVersions().Contains(requestedVersion) &&
                     !cd.ActionDescriptor.GetApiVersions().Any()));
            ...
            using (var iterator = matches.GetEnumerator())
            {
                ...
                while (iterator.MoveNext())
                {
                    if (iterator.Current.ActionDescriptor.ControllerDescriptor != controllerDescriptor)
                    {
                        throw CreateAmbiguousControllerException(directRouteCandidates);
                    }
                }
