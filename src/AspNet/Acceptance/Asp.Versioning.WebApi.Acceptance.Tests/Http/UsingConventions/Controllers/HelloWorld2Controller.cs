...
public class HelloWorld2Controller : ApiController
{
    ...
    [Route]
    public IHttpActionResult GetV3() => Ok( new { controller = GetType().Name, version = Request.RequestedApiVersion.ToString() } );

    [Route( "{id:int}" )]
    public IHttpActionResult GetV3( int id ) => Ok( new { controller = GetType().Name, id, version = Request.RequestedApiVersion.ToString() } );
}